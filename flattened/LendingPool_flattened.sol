...
// File contracts/IBorrowRegistry.sol
...
contract Staking is ReentrancyGuard, Ownable {
    ...
    // Helper: get APY for a specific stake
    function getStakeAPY(address user, uint256 stakeIndex) external view returns (uint256) {
        require(stakeIndex < stakes[user].length, "Invalid stake index");
...
contract LendingPool is ReentrancyGuard, Ownable {
    ...
    function withdraw(uint256 amount) external nonReentrant {
        if (amount == 0) revert InvalidAmount();
        if (lpTotalDeposits[msg.sender] < amount) revert InsufficientBalance();

        // Check if there's enough liquidity
        uint256 availableLiquidity = totalDeposits - totalBorrowed;
        if (availableLiquidity < amount) revert InsufficientLiquidity();

        // Withdraw from deposit positions FIFO, respecting lockup for Senior
        ...
        // If we couldn't fulfill the withdrawal due to lockup, revert
        if (remaining > 0) {
            revert("Not enough unlocked deposits to withdraw requested amount");
        }
    ...
    function withdrawInterest() external nonReentrant {
        ...
        for (uint256 i = 0; i < deposits.length; i++) {
            ...
            uint256 rate = dep.tranche == Tranche.Junior ? JUNIOR_LP_INTEREST_RATE : SENIOR_LP_INTEREST_RATE;
